...
describe('Rules Engine & Intent Parser Unit Tests', () => {
  ...
  describe('parseUserIntent 3-Step Guided Flow vs Shortcut', () => {
    ...
    test('STEP 3: leave type & days given ("3 din casual leave") without date → triggers ask_leave_date', () => {
      const intentResult = parseUserIntent('3 din casual leave');
      expect(intentResult.intent).toBe('ask_leave_date');
      expect(intentResult.leaveType).toBe('casual');
      expect(intentResult.days).toBe(3);
    });

    test('STEP 4: days & date given ("3 din 15 september se") with pendingLeaveType → triggers apply_leave', () => {
      const intentResult = parseUserIntent('3 din 15 september se', [], false, null, 'casual');
      expect(intentResult.intent).toBe('apply_leave');
      expect(intentResult.leaveType).toBe('casual');
      expect(intentResult.days).toBe(3);
    });

    test('SHORTCUT: single message with leave type, days & date ("Mujhe agle hafte 3 din ki casual leave chahiye") → skips to apply_leave', () => {
