...
export function submitLeaveApplication(
  ...
  const endStr = endDate || new Date(Date.now() + (requestedDays - 1) * 86400000).toISOString().split('T')[0];
  const datesStr = startDate && endDate ? `${startDate} to ${endDate}` : `${startStr} to ${endStr} (${requestedDays} Day${requestedDays > 1 ? 's' : ''})`;
