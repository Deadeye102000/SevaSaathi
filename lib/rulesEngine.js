 ...
 */
/**
 * Helper to generate explicit list of consecutive dates (e.g. "15, 16, 17, 18, 19 September 2026")
 */
export function formatExplicitDaysList(startDateStr, daysCount = 1) {
  const days = Math.max(1, Number(daysCount) || 1);
  let baseDate = new Date('2026-09-15');
  ...
  return `${dayNumbers.join(', ')} ${startMonth} ${year}`;
}
...
export function submitLeaveApplication(
  ...
  const startStr = startDate || '2026-09-15';
  const endStr = endDate || new Date(new Date(startStr).getTime() + (requestedDays - 1) * 86400000).toISOString().split('T')[0];
  const explicitList = formatExplicitDaysList(startStr, requestedDays);
  const datesStr = `${explicitList} (${requestedDays} Day${requestedDays > 1 ? 's' : ''})`;

  return {
    application_id: `LV-2026-${randomFourDigits}`,
    id: `LV-2026-${randomFourDigits}`,
    status: 'Submitted',
    routed_to: routedTo,
    reporting_officer: routedTo,
    leaveType: normalizedType,
    type: normalizedType,
    days: requestedDays,
    startDate: startStr,
